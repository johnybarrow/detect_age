...
async def stop_game(msg: types.Message):
    if msg.from_user.id in players_array:
        ...
        if score == AI_score:
            frase = 'Ничья! Похоже, ИИ можно доверять\nСыграете еще? /game'
        elif score < AI_score:
            frase = 'Ух ты! Вы одолели ИИ! Хоть ИИ и силен но, видимо, еще не скоро он сможет заменить людей\nА еще раз сможете? /game'
        else:
            frase = 'Вы проиграли ИИ. Похоже, ему можно доверять\nПопробуйте еще раз /game'
...
async def process_start_command(msg: types.Message):
    ...
    await bot.send_message(msg.from_user.id,
                           f"Просто отправьте мне *фото человека* (например, Ваше 😄, или Вашего двоюродного дедушки), "
                           f"а я, используя свою "
